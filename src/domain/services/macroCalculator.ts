 ...
 * manutenção (TDEE) → alvo (TDEE × ajuste do objetivo) → proteína → gordura
 * → carboidrato → fibra. Devolve o raciocínio junto (não só os números
 * finais), pra dar transparência sobre de onde cada meta veio.
 */
export function calculateMacroTarget(bio: UserBiometrics, goal: Goal): MacroTarget {
  // 1. Calorias — manutenção (TDEE) e depois o alvo, aplicando o ajuste do
  // objetivo (ex.: cutting = TDEE × 0.80, hipertrofia = TDEE × 1.08)
  const maintenanceCalories = calculateTDEE(bio);
  const targetCalories = Math.round(
    maintenanceCalories * (1 + goal.calorieAdjustmentPct / 100)
  );
  ...
  // topo da faixa (ex.: faixa 1.6-2.2 do objetivo "ganhar massa" — sedentária
  // sem treino fica perto de 1.6, ativa treinando bastante perto de 2.2).
  const proteinPerKg =
    goal.proteinGKgMin +
  ...
  // 3. Gordura — % das calorias-alvo definido pelo objetivo (a maioria usa
  // 27%, dentro da faixa 20-35% geralmente recomendada; cutting usa 23%,
  // mais baixo, dentro da sua faixa própria de 15-30%), com piso de 0.8g/kg
  // pra não cair abaixo do mínimo saudável em déficits mais agressivos
  const fatFromPct = Math.round((targetCalories * goal.fatPctOfCalories) / 100 / 9);
  ...
  // 5. Fibra — 14g por 1000kcal das calorias-alvo, sem piso fixo por
  // idade/sexo: uma pessoa com meta de 1.500kcal recebe ~21g, não um valor
  // genérico de 28g "pra todo mundo". A tolerância entre o mínimo aceitável
  // e o ideal já é tratada à parte (ver DailyTargetRange, em planGenerator.ts).
  const fiberG = Math.round((targetCalories / 1000) * 14);

  return {
    maintenanceCalories: Math.round(maintenanceCalories),
    targetCalories,
    calorieAdjustmentPct: goal.calorieAdjustmentPct,
    goalSlug: goal.slug,
    proteinG,
    carbsG,
    fatG,
